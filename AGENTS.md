...
> **Project:** multiterm-astro — Terminal-inspired Astro blog
...
When the user asks to debug in browser / preview / 浏览器调试:
...
1. **Start dev server in a new terminal:**
   ...
   ```
2. **Wait for the server** — poll port 4321 (IPv6 `[::1]`):
   ```powershell
   netstat -ano | Select-String "4321.*LISTENING"
   ```
3. **Open in browser** via Kimi WebBridge (`http://[::1]:4321`)
4. **Verify** with `evaluate` — check `document.title === 'Macondo'`
...
It provides MCP tools for structural code queries — significantly faster than grep/glob.
...
| "Explain how X works" / architecture questions | `codegraph_context` → `codegraph_explore` |

### Rules

- **Answer structural questions directly** with codegraph — don't spawn sub-agents for exploration
- **Don't grep first** for symbol lookup — `codegraph_search` is faster and more accurate
...
| Plan | **Superpowers** | Brainstorming → design → implementation planning |
...
- **When redesigning UI/interface: automatically invoke `impeccable` skill** for evaluation, design, implementation, and review (assess → plan → modify → audit). Never touch visual design without going through impeccable first.
...
- Group order: Astro core → Components → Utils → Types
...
- **SSG data fetching** (GitHub Calendar, iNaturalist): build-time `fetch()` in Astro frontmatter with `AbortController` timeout. On failure, render nothing — never block the build
- **Configuration-driven features**: Add config to `site.config.ts` → types in `types.ts` → consume in components. Use `siteConfig` for site-wide defaults, content frontmatter for per-page overrides
...
| `remark-gemoji` | Remark | `:smile:` → emoji |
...
Markdown Source
  │
  ├─ Remark Phase
  │   ├── remark-reading-time     → Web RSS
  │   ├── remark-description      → Web RSS
  │   ├── remark-directive        → Web RSS
  │   ├── remark-admonitions      → Web RSS
  │   ├── remark-character-dialogue → Web
  │   ├── remark-unknown-directives → Web
  │   ├── remark-gemoji           → Web RSS
  │   └── remark-math             → Web RSS
  │
  ├─ Rehype Phase
  │   ├── rehype-heading-ids      → Web RSS
  │   ├── rehype-autolink-headings → Web RSS
  │   ├── rehype-title-figure     → Web
  │   ├── rehype-external-links   → Web RSS
  │   ├── rehype-unwrap-images    → Web RSS
  │   ├── rehype-pixelated        → Web
  │   ├── rehype-cdn-image        → Web (auto lazy load CDN images)
  │   └── rehype-katex            → Web RSS
  │
  └─ Exports
      ├── Web HTML (all plugins)
      ├── RSS XML (remarkReadingTime + basic markdown-it rendering)
      └── OG Social Cards (plain text, satori rendering)
```
...
```
src/
├── components/         # Reusable Astro components (27 files)
├── content/            # Markdown: posts/, memos/, home.md, addendum.md
├── layouts/            # Layout.astro, MarkdownLayout.astro
├── pages/              # File-based routes + RSS + social cards
├── plugins/            # Remark/Rehype plugins
├── styles/             # global.css (Tailwind + theme vars + prose)
├── icons/              # SVG icons (kebab-case)
├── templates/          # post-template.md, memo-template.md
├── types.ts            # All TypeScript types
├── utils.ts            # Sorting, collation groups, theme resolution
└── site.config.ts      # Central config: site meta, nav, themes, inaturalist
...
- **Overrides** via `site.config.ts` `themes.overrides` — map any theme key to a hex color or another theme key
...
**⚠️ Critical Rule:** After code completion and verification, **always ask for user approval before any git commit or merge**. Never auto-commit or auto-merge without explicit user consent.

```bash
# Only after user says "commit", "提交", "merge", "并入" etc.:
...
```
git diff dev...HEAD → identify affected files by type (.astro/.ts/.css/.md)
       │
       ▼
Run automated checks → pnpm check + tsc --noEmit (if .ts changed)
       │
       ▼
pnpm build → verify success (exit 0, no "error"/"Error" in output)
       │
       ▼
Post-build: orphaned brackets, internal links, title uniqueness, external link rel
       │
       ▼
Agent inspection: route coverage, responsive, 14 themes, subjective UX
       │
       ▼
Report → structured pass/fail verdict
```
...
| Event | Scope |
|-------|-------|
| New feature branch | Full QA (automated → build → post-build → visual) |
...
```
site.config.ts → types.ts → content.config.ts (if content) → component consumer
```
...
```typescript
// src/music-metadata.ts — two-step fallback:
// 1. Parse iTunes URL for song ID (?i=SONG_ID)
// 2. Call itunes.apple.com/lookup?id=SONG_ID for artwork + artist
```
Used by `MusicCard.astro`. Never embed Apple Music iframes — use link cards with metadata.
...
4. `pnpm build` to verify — don't rely on visual review alone
...
**Content separation**: Personal content (`src/content/posts/`, `src/content/memos/`) is gitignored on GitHub; only synced to Gitea. Vercel auto-deploy is disabled via `vercel.json`.

### Vercel Deploy Pipeline

```
dev.ps1:
  node scripts/qa.mjs    → source checks + Vercel build + post-build checks (single pass)
  vercel deploy --prebuilt → upload pre-built output (624KB, no remote build)
```

No duplicate builds — QA inspects `.vercel/output/static/` (same output deployed to production).
